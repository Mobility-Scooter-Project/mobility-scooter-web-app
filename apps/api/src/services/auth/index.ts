...
export class AuthService {
  ...
  public constructor(
    ...
    @inject(SMTPService) smtpService: SMTPService,
  ) {
   ...
   */
  public async signInWithPassword(db: DB, email: string, password: string) {
    let user;
    try {
   ...
   */
  public async refreshToken(db: DB, refreshToken: string) {
   ...
   * @returns In non-production environments, returns the generated token. In production, returns undefined.
   ...
   */
  public async generateResetPasswordToken(email: string): Promise<string | undefined> {
    let data;
    try {
