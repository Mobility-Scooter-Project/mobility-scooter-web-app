...
const app = new Hono<{ Variables: Variables }>().post('/videos/presigned-url', dbMiddleware, validateApiKey, zValidator("json", presignedUrlSchema), async (c) => {
    const { filename, patientId, date } = c.req.valid("json")
    const userId = c.get("userId")!;

    const url = await storageService.generatePresignedVideoUrl(filename, userId, patientId, date);
