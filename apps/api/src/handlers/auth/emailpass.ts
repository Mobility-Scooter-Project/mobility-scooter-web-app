...
import { signUpRateLimiter, signInRateLimiter, resetPasswordRateLimiter } from "@src/middleware/rate-limit";
import { validateApiKey } from "@src/middleware/validate-api-key";
import { authService } from "@src/services/auth";
import { createUserWithPasswordSchema, signInWithPasswordSchema, resetPasswordTokenSchema, resetPasswordSchema } from "@src/validators/auth";
...
const app = new Hono<{ Variables: Variables }>().post(
    ...
    .post(
    ...
    ).post("/reset-password/token", validateApiKey, zValidator("json", resetPasswordTokenSchema), resetPasswordRateLimiter, async (c) => {
        ...
        return c.text("OK");
    }).post("/reset-password", validateApiKey, zValidator("json", resetPasswordSchema), resetPasswordRateLimiter, async (c) => {
